...
### 🔎 Puente A — herramienta READ_ONLY de lectura de archivos (abierta a todos los usuarios)
Vectrax puede leer un archivo específico del propio repositorio —o resolver un símbolo de código conocido (`Clase.método()`)— y responder desde esa evidencia real, sin cargar el repo completo y sin que el LLM decida qué herramienta existe. Primera pieza de una arquitectura de puentes de herramientas (Puente A = solo lectura; Puente B, futuro = escrituras reversibles, gobernado por la propiedad `reversibility` del catálogo de capacidades). Cerrado y abierto a **todos** los usuarios (no solo el creador) tras validación end-to-end real en producción.

**Pipeline**: `Usuario → Intent determinista → Capability gate → local_filesystem.read → evidencia → respuesta`.

- **Intent determinista** (`core/operator/read_tool_intent.py`) — dos patrones, ambos regex acotados (es/en), sin LLM decidiendo herramientas:
  - Ruta explícita: "abre/lee/muéstrame `<path>` ... qué hace `<función>`". Extrae ruta relativa + símbolo opcional.
  - Símbolo sin ruta: "qué hace/explica/what does `<Clase>.<método>()`" o función/clase suelta — se resuelve vía `read_tool_bridge._resolve_symbol_location()` buscando **solo** en `sys.modules` ya cargados en el proceso (nunca escanea el filesystem, nunca importa a ciegas, nunca inventa una ruta si no la encuentra).
  - Ambos rechazan rutas absolutas, traversal (`..`), extensiones no-texto (`.db`, `.png`, etc.), **y rutas protegidas** (`is_protected_path()`: `vault/`, `keys/`, `secrets/`, `.git/`, `.ssh/`, `.env`, `.venv/` — por segmento completo de ruta, no substring) aunque tengan una extensión permitida.
- **Capability gate** (`core/self_observation/capability_context.py`) — campo `reversibility` en `CapabilityEntry` (`READ_ONLY` / `REVERSIBLE_WRITE` / `BEHAVIOR_CHANGE` / `IRREVERSIBLE_CRITICAL`, default `UNCLASSIFIED`). Solo `local_filesystem` está clasificada `READ_ONLY` por ahora — el resto del catálogo queda sin clasificar hasta que un puente futuro lo necesite.
- **Ejecución** (`core/operator/read_tool_bridge.py`) — reutiliza `LocalFilesystemConnector.read()` / `ConnectionEngine.read()` y su sandbox (`_safe_path()`, confinado a la raíz del repo) ya existentes en la Universal Connector Engine (UCE); ningún lector nuevo, ningún permiso ampliado (solo `"read"`). Si se pidió un símbolo, lo extrae vía `ast` (determinista, nunca ejecuta el código leído; acotado a la clase específica cuando se conoce, evitando métodos homónimos).
...
- **Alcance**: gateado Únicamente por `VX_TOOL_BRIDGE_READ_ONLY` (default OFF — cero cambio de comportamiento hasta activarlo). Sin restricción por identidad de usuario — la seguridad depende de `_safe_path()`, `is_protected_path()`, la whitelist de extensiones y el permiso único `"read"`, no de quién pregunta.
...
Tests: `tests/test_read_tool_bridge.py` (31 tests: parser positivo/negativo por ruta y por símbolo, path traversal, extensión no-texto, rutas protegidas con extensión permitida, nombres parecidos no bloqueados falsamente, capability gate, conteo real de llamadas `read()`/`write()` vía el tracker de la UCE, variación de estado real entre lecturas, símbolo inexistente nunca inventa, resolución end-to-end contra `connectors/engine.py` real, verificación estática de que el gate por creador fue removido, flag OFF sin cambios)
...
### 2026-09-13
- **fix(criterion): regresión de evidencia por dominio (freight/cybersecurity/real_estate) — `known_domains()` incompleto** (commit `878286c`) — Reporte de producción: `cybersecurity`/`florida_real_estate` dejaron de aparecer entre los dominios de Gravity/Universo, y "¿Qué has observado en el dominio de ciberseguridad?" se respondía con evidencia de `market`. Investigado con evidencia (no supuesto): los dos commits anteriores de la sesión (`900336c`, `ade756a`) NO tocan `criterion.py`/`gravity_engine.py`/`domain_knowledge.py`/`verification_ledger.py`, y `known_domains()` no cambia desde 2026-07-15 — el defecto era preexistente. Causa raíz real: `known_domains()` solo unía `gravity_engine.domain_stats()` + `domain_knowledge.list_domains()`; `cybersecurity` nunca tuvo registros en el primero ni archivo en el segundo, así que `detect_domain()` jamás podía devolverlo aunque la evidencia real existía en `verification_ledger/cybersecurity.jsonl` (168MB) y `build_criterion_result()` la recuperaba correctamente cuando se le pasaba el dominio explícito. Fix: nuevo `verification_ledger.list_domains()` sumado como tercera fuente de `known_domains()` (fail-safe). Investigación separada del vacío de `florida_real_estate` en `verification_ledger`: `RentCastProvider` solo produce el evento `sale_closed` (única "verdad objetiva" aceptada) detrás de `RENTCAST_INCLUDE_SOLD` (default OFF, ahorro de cuota) — verificado en vivo contra la API real (20/20 eventos devueltos fueron `new_listing`, cero `sale_closed`); fix disponible (`RENTCAST_INCLUDE_SOLD=1`) pendiente de decisión por costo de cuota. Regresión de 5 puntos ejecutada DOS veces contra el pipeline real (13 y 17 de septiembre): freight/cybersecurity/real_estate/market cada uno con su propia evidencia sin contaminación cruzada, Puente A y Reasoning Engine funcionando, resultados idénticos entre corridas. +8 tests. Cierre: `docs/DOMAIN_EVIDENCE_REGRESSION_2026_09_13.md`.
- **feat(operator): Puente A — cierre final, abierto a TODOS los usuarios** (commit `456207a`) — Retirada la restricción creator-only del STEP 4.2a2b tras validación end-to-end real en Telegram. Antes de abrirlo, se cerró una brecha real: la whitelist de extensiones (`.json`/`.yaml`/`.txt`) no bloqueaba por sí sola rutas como `keys/api_key.json` o `secrets/token.json` (viven DENTRO del sandbox de `_safe_path()`, que solo impide escapar de él). Nuevo `is_protected_path()` (denylist por segmento: `vault/`, `keys/`, `secrets/`, `.git/`, `.ssh/`, `.env`, `.venv/`) aplicado tanto en el parser de ruta explícita como en la resolución por símbolo. Verificadas las 6 pruebas obligatorias contra el pipeline real: creador sigue funcionando, dos usuarios no-creador distintos activan Puente A (ruta explícita y símbolo sin ruta) y obtienen evidencia real, 4 intentos contra rutas protegidas rechazados (0 lecturas en los 4), READS=1/WRITES=0 en cada caso exitoso, `/health` ok. +7 tests. Cierre: `docs/PUENTE_A_CLOSURE_2026_09_13.md`.
- **fix(operator): Puente A — reconocer preguntas por símbolo de código** (commit `8928c85`) — Defecto confirmado: "¿Qué hace ConnectionEngine.read() en tu código?" no activaba Puente A (solo reconocía "abre/lee `<ruta>`") y caía a búsqueda online, produciendo una respuesta genérica e incorrecta sobre sockets. Nuevo `parse_symbol_lookup_request()` + `_resolve_symbol_location()` (resuelve el símbolo buscando SOLO en `sys.modules` ya cargados, nunca escanea el filesystem, nunca inventa una ruta). La ruta resuelta se entrega al mismo `local_filesystem.read()`/`ConnectionEngine.read()` de siempre. Verificado: la pregunta ahora activa Puente A, 1 `read()`, 0 `write()`, respuesta fiel a `connectors/engine.py` (permission-check, timing, normalize, track_operation) en vez de la respuesta genérica anterior. +8 tests.
- **feat(operator): Puente A — herramienta READ_ONLY de lectura de archivos con evidencia** (commit `9d97fba`) — `Usuario → Intent determinista → Capability gate → local_filesystem.read → evidencia → respuesta`, gateado por `VX_TOOL_BRIDGE_READ_ONLY` (default OFF) y restringido al creador en esta primera versión. Nuevo campo `reversibility` en `CapabilityEntry` (`READ_ONLY`/`REVERSIBLE_WRITE`/`BEHAVIOR_CHANGE`/`IRREVERSIBLE_CRITICAL`), catalogando `local_filesystem` como `READ_ONLY`. Reutiliza `LocalFilesystemConnector.read()`/`ConnectionEngine.read()` y su sandbox (`_safe_path()`) ya existentes — ningún lector nuevo, ningún permiso ampliado. Extracción de símbolo vía `ast`; composición narrativa con anclaje estricto (el LLM redacta tono, nunca hechos; fallback a cita literal si no puede ser fiel). Verificado end-to-end contra el pipeline real: 1 sola llamada `read()`, 0 `write()`, respuesta con evidencia real de `system_monitor.py::collect_metrics()`. Nueva sección *Puente A*. `tests/test_read_tool_bridge.py` (18 tests). Ver issue de cierre para el detalle de despliegue.
...
- **feat(resiliencia): watchdog de PROGRESO del gateway** (PR #57) — El watchdog previo vigilaba `_last_poll_ok`, que el kill-path refresca cada ~40s → NUNCA detectó el deadlock (proceso vivo, 0 progreso, 20h). Nuevo watchdog en `_heartbeat_loop` sobre `self._polls` (solo sube en poll exitoso): sin avance en `POLL_PROGRESS_STALL_THRESHOLD` (180s) → `os._exit(1)` para reinicio limpio del supervisor. `NET_DOWN` cuenta como progreso (no reinicia en outage de red real).
- **fix(resiliencia): Self-Audit Engine revivido en macOS** (PR #57) — `audit_cron` usaba `cron -f` (cron del sistema) → SIGKILL/`exit=-9` en Mac → el auditor nunca corría. Ahora: `audit_engine` con `check_container`/`check_processes` cross-platform (fallback `pgrep`/`ps` cuando no hay `/proc`), modo `audit_cron --loop` (scheduler Python defensivo + `load_dotenv`), y el supervisor lanza `python -m observability.audit_cron --loop`. Verificado en vivo: `startup daily audit: ESTABLE`, sin `exit=-9`.
- **ci: quality gate activado y en verde** (PRs #58–#60) — `.github/workflows/ci.yml` corre la suite hermética (`pytest -m "not live"`, Python 3.9) en cada push a `main` y en cada PR. Para llegar a verde: declaradas deps reales no listadas (`sqlalchemy`, `python-multipart`, `aiosqlite`); los tests de auth usan un `VX_API_TOKEN` fuerte vía `monkeypatch` (sin reabrir el backdoor eliminado; `test_legacy_token_fallback` reescrito como guard de seguridad); `test_phase3` marcado `live` (requiere Ollama); `code_reader`/`integrator`/`sandbox_validator` derivan `BASE_DIR` del `__file__` (antes `~/Vectrax` hardcodeado rompía fuera del home — CI/Docker); el bloque Router de `_collect_module_state` se emite siempre (defaults en entorno fresco). Resultado en `main`: **3092 passed, 1 skipped, 6 deselected (`live`), 0 failed**. Cierre: `docs/RESILIENCE_CI_CLOSURE_2026_07_26.md`.
### 2026-07-23
- **test(aislamiento): prueba de carga multiusuario — 0 fugas de contexto** — Verificación de que Vectrax no mezcla contextos entre usuarios bajo concurrencia real. Dos pruebas con `threading.Barrier`: (1) end-to-end gateway, 8 usuarios × 3 rondas (24 respuestas simultáneas) → `AISLAMIENTO INTACTO (0 fugas)`; (2) resolver de memoria (`resolve_with_memory`), 10 usuarios × 5 rondas (50 resoluciones simultáneas) → `own_missing=0` (atribución positiva 100% — cada usuario recibe SU propio nombre) + `fugas=0`. Memoria correctamente segmentada por `user_id`; el acceso concurrente al store mantiene la integridad. Config de voz (`.env`, gitignored): `VECTRAX_AUDIO_MODE=audio` porque la cuenta rechaza `sendVoice` (`400 VOICE_MESSAGES_FORBIDDEN`) → usa `sendAudio` (MP3). Cierre: `docs/CONTEXT_ISOLATION_LOAD_TEST_2026_07_23.md`.
