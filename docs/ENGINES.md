...
- 🔴 **Memory Governor** · `core/gravity/governor.py` (376 loc) — memoria infinita → significativa; protege identidad, aísla por `user_id`, evita entropía. **Estado real (auditoría 2026-09-11, ver `docs/CIRCUIT_AUDIT_2026_09_11.md`): BLOQUEADO.** `MemoryGovernor(` no se instancia en ningún archivo de producción, y `~/.vectrax/gravity.db` no tiene la tabla `deep_memory` — todo el subsistema de memoria profunda de este grupo (Vector Store, Retrieval, Concept Fusion, Essential Summary, Memory Decay) está desconectado de producción, sin escritor ni lector reales. Pendiente de auditoría separada para decidir si es un camino paralelo, obsoleto, o a integrar con la memoria real (`vectrax/user_memory.py` / `core/learn/gravity_engine.py`).
...
- 🔴 **Convergence History** · `core/learn/convergence_history.py` (262 loc) — nacimiento/evolución/disolución de convergencias. API: `record_snapshot`, `get_active`, `get_timeline(days)`, `build_context`. **Estado real: huérfano.** Grep exhaustivo (auditoría 2026-09-11) no encontró ningún invocador de producción de estas funciones — solo se usan en tests. La autoridad real de convergencias es `core.learn.convergence_registry` (consumida por `universe_census`).
...
- 🟢 **Convergence Learner** · `core/nucleus/convergence_learner.py` (667 loc) — cierra la conciencia operacional: observa decisiones del observer + resultado posterior (`IMPROVED / NEUTRAL / DEGRADED`), detecta patrones por motor y recomienda ajustes de umbral con evidencia. API: `get_learner()`, `ConvergenceLearner` (`record_decision`, `record_outcome`, `analyze`, `generate_recommendations`, `approve_recommendation`, `reject_recommendation`, `advance_phase`). **Estado real (auditoría 2026-09-11):** `record_decision()` se invoca en cada evaluación real de `PresenciaObserver`; `record_outcome()` estuvo huérfano (sin invocador) hasta esa fecha — ahora conectado al Response Auditor de `external_gateway.py` (rama de violación de leyes), probado con avance real OBSERVE→LEARN→RECOMMEND. `analyze()`/`generate_recommendations()`/`advance_phase()` funcionan correctamente con datos reales pero **no tienen invocador automático** — solo se disparan manualmente; aplicar cambios sigue requiriendo autorización del creador.
...
- 🟢 **Active Learning Orchestrator** · `core/learn/active_learning.py` (705 loc) — conecta todos los subsistemas en un ciclo autónomo (detectar patrones → generar → integrar). API: `get_orchestrator()`, `ActiveLearningOrchestrator` (`activate`, `deactivate`, `is_active`, `run_cycle`, `on_file_ingested`, `status`). **Estado real (auditoría 2026-09-11):** `run_cycle()` estuvo huérfano (sin invocador de producción; su único trigger event-driven dependía de un filesystem watchdog no supervisado) hasta esa fecha — ahora conectado al main loop de `core/transport/pipeline_worker.py`, corre cada 30 min si `is_active()` (gate propio del orchestrator).
...
- 🔵 **Router Learning Cycle** · `core/router_learning_cycle.py` (444 loc) — detecta conflictos recurrentes (semántico vs regex) y genera **propuestas** de mejora (no aplica sin aprobación). API: `RouterLearningCycle` (`activate`, `deactivate`, `is_active`, `run_cycle`, `get_pending_proposals`). **Estado real (auditoría 2026-09-11):** ON-DEMAND — sin loop ni scheduler propio; se invoca manualmente vía `vx router-learn cycle` (docstring corregido, antes afirmaba incorrectamente ser "continuo").
...
**Estado del grupo:** el aprendizaje selectivo (gate/refinement/bias) está activo en el flujo de ingest. `active_learning` corre automáticamente cada 30 min desde `pipeline_worker.py` (conectado 2026-09-11); `router_learning_cycle` es on-demand vía CLI. Ambos **generan propuestas/reglas sin aplicar cambios críticos** sin autorización — coherente con "aprender solo de patrones confirmados".
