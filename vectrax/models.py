...
# ---------------------------------------------------------------------------
# StarProvenance — PARTE 2 del contrato de memoria conversacional
# (derivacion selectiva de estrellas personales con procedencia)
# ---------------------------------------------------------------------------

# Categorias significativas -- una estrella personal SOLO se crea/actualiza
# cuando el mensaje encaja en una de estas categorias genericas (nunca por
# frases cerradas de un caso particular ni nombres propios hardcodeados).
STAR_CATEGORY_IDENTITY = "identity"
STAR_CATEGORY_RELATION = "relation"
STAR_CATEGORY_PREFERENCE = "preference"
STAR_CATEGORY_PROJECT = "project"
STAR_CATEGORY_DECISION = "decision"
STAR_CATEGORY_COMMITMENT = "commitment"
STAR_CATEGORY_GOAL = "goal"
STAR_CATEGORY_FACT = "fact"
STAR_CATEGORY_STATE_CHANGE = "state_change"

# Estados del ciclo de vida de una afirmacion personal derivada.
# Las correcciones marcan sustitucion, NUNCA se borra el registro anterior.
STAR_PROVENANCE_ACTIVE = "active"          # afirmacion vigente actual
STAR_PROVENANCE_SUPERSEDED = "superseded"  # reemplazada por una correccion explicita del propio usuario
STAR_PROVENANCE_CONTRADICTED = "contradicted"  # dos afirmaciones distintas sin correccion explicita -- conflicto honesto
STAR_PROVENANCE_UNCERTAIN = "uncertain"    # extraccion de baja confianza, pendiente de confirmacion

STAR_PROVENANCE_STATUSES = (
    STAR_PROVENANCE_ACTIVE,
    STAR_PROVENANCE_SUPERSEDED,
    STAR_PROVENANCE_CONTRADICTED,
    STAR_PROVENANCE_UNCERTAIN,
)

STAR_EXTRACTOR_VERSION = 1


@dataclass
class StarProvenance:
    """Procedencia de una estrella personal derivada selectivamente.

    Nunca se deriva desde busqueda web ni desde inferencia del LLM sin
    confirmacion explicita del usuario -- solo desde texto literal escrito
    por el propio usuario (ledger canonico, PARTE 1).
    """
    ...
    channel: str = CHANNEL_USER
    owner: str = ""
    category: str = STAR_CATEGORY_FACT
    entity: str = ""                          # tema/entidad normalizado (p.ej. "novia", "proyecto")
    normalized_assertion: str = ""            # afirmacion normalizada (minusculas, sin espacios extra)
    source_event_ids: List[str] = field(default_factory=list)  # ids del ledger canonico (PARTE 1)
    literal_fragments: List[str] = field(default_factory=list)  # texto literal original, nunca reescrito
    confidence: float = 0.0
    status: str = STAR_PROVENANCE_ACTIVE
    extractor_version: int = STAR_EXTRACTOR_VERSION
    previous_star_id: str = ""                # si status='superseded'/'contradicted': la estrella que reemplaza
    id: str = field(default_factory=lambda: str(uuid.uuid4()))
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "literal_fragments": list(self.literal_fragments),
            "confidence": round(self.confidence, 4),
            "status": self.status,
            "extractor_version": self.extractor_version,
            "previous_star_id": self.previous_star_id,
